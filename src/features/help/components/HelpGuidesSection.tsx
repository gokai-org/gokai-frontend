...
interface HelpGuidesSectionProps {
  onStartGuide: (tourId: string) => void;
...
export function HelpGuidesSection({
  ...
  const visibleGuides = HELP_GUIDES.filter((guide) => {
    if (guide.tourId !== "chatbot-guide") {
      return true;
    }

    return accessResolved && isPremium;
  });
    ...
    <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
      {visibleGuides.map((guide, index) => {
        const card = (
          <button
            key={guide.title}
            onClick={() => onStartGuide(guide.tourId)}
