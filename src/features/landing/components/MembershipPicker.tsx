...
export default function MembershipPicker({
  ...
  return (
    <div
      className={[
        "mx-auto grid max-w-5xl grid-cols-1 gap-8 md:grid-cols-2",
        className,
      ].join(" ")}
    >
      {PLANS.map((plan, idx) => {
            ...
            <div className="bg-white px-8 pb-8 pt-7">
              <div className="rounded-2xl bg-white ring-1 ring-black/10 shadow-sm p-5">
                <div className="flex items-end gap-3">
                  <span className="text-neutral-300 text-2xl font-extrabold">
                    $
                  </span>
        ...
        if (mode === "link") {
          return (
            <div
              ...
              onClick={() => {
                router.replace(
                  `/auth/login?mode=register&intent=${plan.intent}`,
                );
              }}
              role="link"
            >
