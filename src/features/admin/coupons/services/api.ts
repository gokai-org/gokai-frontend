...
import type { BackendCoupon, CreateCouponRequest, UpdateCouponRequest } from "../types/coupons";

export async function getAdminCoupons(): Promise<BackendCoupon[]> {
  return apiFetch<BackendCoupon[]>("/admin/api/coupons", { method: "GET" });
}

export async function createAdminCoupon(payload: CreateCouponRequest): Promise<{ success: boolean }> {
...
export async function updateAdminCoupon(id: string, payload: UpdateCouponRequest): Promise<{ success: boolean }> {
