...
export function KanjiLessonResultSummary({
  ...
  const grade = useMemo(() => {
    if (overallScore >= 90) return { label: "¡Excelente!", color: "text-emerald-600 dark:text-emerald-400", ring: "bg-emerald-400/20" };
    if (overallScore >= 70) return { label: "¡Bien hecho!", color: "text-blue-600 dark:text-blue-400", ring: "bg-blue-400/20" };
    if (overallScore >= 50) return { label: "Aceptable", color: "text-amber-600 dark:text-amber-400", ring: "bg-amber-400/20" };
    return { label: "Sigue practicando", color: "text-orange-600 dark:text-orange-400", ring: "bg-orange-400/20" };
  }, [overallScore]);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative"
      >
        <div className={`absolute inset-[-8px] rounded-full blur-xl ${grade.ring}`} />
        <div className="relative w-28 h-28 rounded-full flex flex-col items-center justify-center">
          <svg className="absolute inset-0 w-28 h-28 -rotate-90" viewBox="0 0 112 112">
            <circle cx="56" cy="56" r="50" fill="none" stroke="var(--border-primary)" strokeWidth="6" />
            <motion.circle
      ...
      >
        ...
        <p className="text-sm text-content-tertiary">
          ...
          <span className="font-bold text-content-primary text-lg">{symbol}</span>
      ...
      >
        <div className="text-center px-5 py-3">
          <p className="text-lg font-extrabold text-content-primary">{results.length}</p>
          <p className="text-[10px] text-content-muted font-semibold">Ejercicios</p>
        </div>
        <div className="w-px h-10 bg-surface-tertiary" />
        <div className="text-center px-5 py-3">
          <p className="text-lg font-extrabold text-content-primary">{totalCorrect}/{totalQuestions}</p>
          <p className="text-[10px] text-content-muted font-semibold">Correctas</p>
        </div>
        <div className="w-px h-10 bg-surface-tertiary" />
        <div className="text-center px-5 py-3">
          <p className="text-lg font-extrabold text-content-primary">{overallScore}%</p>
          <p className="text-[10px] text-content-muted font-semibold">Promedio</p>
        </div>
      </motion.div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold">{EXERCISE_LABELS[r.type]}</span>
              </div>
