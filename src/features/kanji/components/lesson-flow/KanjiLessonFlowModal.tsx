...
export function KanjiLessonFlowModal({
  kanji,
  onClose,
}: KanjiLessonFlowModalProps) {
            ...
            {(state.step === "exercise" ||
              ...
              flowData && (
                <AnimatePresence mode="wait">
                  <div
                    key={`${state.currentExerciseIndex}-${state.currentQuestionIndex}`}
                  >
                    {/* Choice-based exercises */}
                    {currentExercise.type === "meaning" && (
                      <KanjiMeaningExercise
                        question={
                          currentExercise.questions[state.currentQuestionIndex]
                        }
...
function LessonIntro({
  ...
  const exerciseDescriptions = [
    {
      icon: "",
      label: "Significado",
      desc: "Identifica el significado del kanji",
    },
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-5xl font-bold text-content-primary select-none">
          {symbol}
        </span>
      </motion.div>
