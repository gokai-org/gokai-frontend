...
function SectionCard({
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="rounded-2xl bg-white ring-1 ring-black/[0.04] shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4"
    >
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <span className="ml-auto text-[11px] font-semibold text-neutral-400 bg-neutral-100 rounded-full px-2 py-0.5">
          {count}
        </span>
      </div>
      {children}
                ...
                <div className="flex flex-wrap gap-2">
                  {readings.length > 0 ? (
                    readings.map((reading, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center rounded-lg border border-blue-100 bg-gradient-to-b from-blue-50 to-blue-50/40 px-3 py-1.5 text-sm font-semibold text-blue-700 shadow-sm shadow-blue-100/50"
                      >
                        {reading}
                      </span>
                    ))
                ...
                <div className="flex flex-wrap gap-2">
                  {meanings.length > 0 ? (
                    meanings.map((meaning, idx) => (
                      <span
                        key={idx}
                        className="inline-flex items-center rounded-lg border border-emerald-100 bg-gradient-to-b from-emerald-50 to-emerald-50/40 px-3 py-1.5 text-sm font-semibold text-emerald-700 capitalize shadow-sm shadow-emerald-100/50"
                      >
                        {meaning}
                      </span>
                    ))
