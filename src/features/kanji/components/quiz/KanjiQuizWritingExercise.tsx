...
interface KanjiQuizWritingExerciseProps {
...
export function KanjiQuizWritingExercise({
  question,
  questionIndex,
    ...
    return (
      ...
      >
        <p className="text-content-tertiary text-sm text-center">
          Datos de trazos no disponibles para <strong>{question.kanji}</strong>.
        </p>
        <button
  ...
  if (phase === "demo") {
    return (
      <motion.div
        key={`writing-demo-${questionIndex}`}
      ...
      >
        ...
        {totalWritingQuestions > 1 && (
          <div className="flex items-center gap-1.5">
            {Array.from({ length: totalWritingQuestions }).map((_, i) => (
              <div
                key={i}
  ...
  if (phase === "practice") {
    return (
      <motion.div
        key={`writing-practice-${questionIndex}`}
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.35 }}
