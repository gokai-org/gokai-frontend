...
export async function GET(req: NextRequest) {
  ...
  const upstreamPath = `/content/recommendations/subtheme/${userId}?themeId=${encodeURIComponent(themeId)}&limit=${encodeURIComponent(limit)}`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Error interno al cargar recomendaciones de subtemas" },
      { status: 500 },
    );
  }
}
